...
Uses a 14-day sliding window to predict the next day's sales for each
product family. MinMaxScaler is fit on train data only. Saves the trained
model as .h5, training history, and a per-family predictions CSV.

Usage:
    python src/forecasting/model_lstm.py
"""
...
def build_sequences(data, window_size):
    ...
    for i in range(len(data) - window_size):
        ...
        y.append(data[i + window_size, 0])  # first column = aggregated sales
...
def main():
    ...
    for family in families:
        ...
        # Feature matrix: [sales, day_of_week, month, is_weekend]
        ...
        # Build sequences from train; for test, prepend last WINDOW_SIZE train rows
        ...
        # Save model
        model_path = os.path.join(MODELS_DIR, f"lstm_{family.replace(' ', '_')}.h5")
        model.save(model_path)
        model_size_kb = os.path.getsize(model_path) / 1024

        # Save scaler
        scaler_path = os.path.join(MODELS_DIR, f"lstm_scaler_{family.replace(' ', '_')}.pkl")
        ...
        # Inverse transform: reconstruct full feature vector, replace col 0, then inverse
        ...
        # Save training history for loss curve plot
        ...
        history_df.to_csv(
            f"{RESULTS_DIR}/lstm_history_{family.replace(' ', '_')}.csv", index=False
        )
        ...
        pred_df.to_csv(
            f"{RESULTS_DIR}/forecast_lstm_{family.replace(' ', '_')}.csv",
            index=False
        )

    results_df = pd.DataFrame(all_results)
    results_df.to_csv(f"{RESULTS_DIR}/forecast_lstm.csv", index=False)
    ...
    pd.DataFrame([summary]).to_csv(
        f"{RESULTS_DIR}/forecast_lstm_summary.csv", index=False
    )

    print(f"\n  Saved: {RESULTS_DIR}/forecast_lstm.csv")
    print("  [DONE] LSTM model complete.")
